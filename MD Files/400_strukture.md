...
>>> boje_b = [ "crvena", "zelena", "plava" ]>>> boje_a == boje_b True
```
...
>>> print(list(r)) # ali bilo koji range možemo pretvoriti u popis [2, 3, 4, 5, 6, 7, 8, 9]>>> r = range(0, 5)
>>> print(list(r)) [0, 1, 2, 3, 4]>>> r = range(5) # ako pošaljemo samo jedan broj, smatra se da je početni broj 0
>>> print(list(r)) [0, 1, 2, 3, 4]>>> boje = ["crvena", "zelena", "zelena", "plava", "crna"]
>>> broj_boja = len(boje)
...
``` python
>>> if "š" in "Krešimir": print(’"Krešimir" sadrži "š".’)"Krešimir" sadrži "š".>>> boje = ["crvena", "zelena", "zelena", "plava", "crna"]
>>> "zelena" in boje True>>> "žuta" in boje False
```
...
>>> print(s)>>> s = set() # prazan skup
>>> print(s) set()>>> s = "a", "b", "c" # skup s članovima
>>> print(s) "a", "c", "b">>> s = set("ana") # napravi skup od elemenata vrijednosti (npr, tekst)
>>> s "a", "n">>> s = set(["ana"]) # napravi skup od elemenata popisa
>>> s "ana">>> d = "a": 1, "b": 2, "c": 1 # definiraj neki rječnik
